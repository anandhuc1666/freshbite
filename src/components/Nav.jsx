    ...
    useEffect(() => {
        if (user?.id) {
            axios.get(`http://localhost:5005/users/${user.id}`)
                .then(res => {
                    setUser(res.data);
                    setCount(res.data.fav?.length || 0);
                })
        ...
        <div className='nav'>
            ...
            <div className="nav-header3">
                ...
                <Link to={'/Fav'} style={{ textDecoration: 'none', color: 'black' }}>
                    <MdFavorite style={{ fontSize: '1.7em' }} />
                </Link>
                <Link to={'/Cart'}>
                    <FaCartShopping style={{ fontSize: '1.7em', color: 'black' }} />
                </Link>
                <Link to={'/UserAcc'}>
                    <MdAccountCircle style={{ fontSize: '1.7em', color: 'black' }} />
                </Link>
            </div>
        </div>
    )
